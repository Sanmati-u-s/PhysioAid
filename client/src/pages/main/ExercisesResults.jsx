  ...
  const getFilteredExercises = () => {
    ...
    if (filters?.difficulty && filters.difficulty !== "All") {
      exercises = exercises.filter(
        (ex) => ex.difficulty === filters.difficulty
      );
    }
    ...
    <div className="min-h-screen gradient-subtle py-12">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="mb-8 animate-fade-in-up">
          <button
            onClick={() => navigate("/main/exercises")}
            className="text-teal-600 dark:text-teal-400 cursor-pointer hover:text-teal-700 dark:hover:text-teal-300 font-semibold mb-6 flex items-center gap-2 transition-all hover:gap-3"
          >
            ← Back to Selection
          </button>

          <div className="flex items-center gap-4 mb-4 animate-scale-in">
            <div className="w-14 h-14 bg-gradient-to-br from-teal-500 to-emerald-600 rounded-xl flex items-center justify-center animate-float">
              <span className="text-2xl">💪</span>
            </div>
            <div>
              <h1 className="text-4xl font-bold gradient-text">
                {selectedMuscleName} Exercises
              </h1>
        ...
        {filteredExercises.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredExercises.map((exercise, index) => (
              <div
                key={exercise.id}
                className="card-elevated p-6 sm:p-8 border-l-4 border-teal-500 hover-glow-teal animate-fade-in-up"
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="ml-4 shrink-0">
                    <span className="inline-block bg-gradient-to-r from-teal-500 to-emerald-600 text-white text-sm font-semibold px-5 py-2 rounded-full shadow-sm">
                      {exercise.type}
                    </span>
                  </div>
                </div>
                ...
                <div className="mb-6 p-4 bg-teal-50 rounded-lg border-2 border-teal-200">
                  <p className="text-sm text-gray-700">
                    <strong className="text-teal-900">Duration:</strong>{" "}
                  ...
                  <ol className="space-y-3">
                    {exercise.instructions.map((instruction, index) => (
                      <li key={index} className="flex gap-4">
                        <span className="shrink-0 flex items-center justify-center h-8 w-8 rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 text-white font-bold text-sm shadow-sm">
                          {index + 1}
                        </span>
          ...
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              No exercises match your selected filters. Try adjusting your
              criteria.
            </p>
            <button
              onClick={() => navigate("/main/exercises")}
